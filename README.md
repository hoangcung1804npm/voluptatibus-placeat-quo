# @hoangcung1804npm/voluptatibus-placeat-quo

![CI](https://github.com/hoangcung1804npm/voluptatibus-placeat-quo/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@hoangcung1804npm/voluptatibus-placeat-quo.svg?style=flat)](https://www.npmjs.com/package/@hoangcung1804npm/voluptatibus-placeat-quo)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)
[![NPM downloads](https://img.shields.io/npm/dm/@hoangcung1804npm/voluptatibus-placeat-quo.svg?style=flat)](https://www.npmjs.com/package/@hoangcung1804npm/voluptatibus-placeat-quo)


__@hoangcung1804npm/voluptatibus-placeat-quo__ is significantly faster than `JSON.stringify()` for small payloads.
...
@hoangcung1804npm/voluptatibus-placeat-quo requires a [JSON Schema Draft 7](https://json-schema.org/specification-links.html#draft-7) input to generate a fast `stringify` function.
...
@hoangcung1804npm/voluptatibus-placeat-quo array default x 6,243 ops/sec ±0.14% (98 runs sampled)
@hoangcung1804npm/voluptatibus-placeat-quo array json-stringify x 6,261 ops/sec ±0.30% (99 runs sampled)
...
@hoangcung1804npm/voluptatibus-placeat-quo large array default x 99.96 ops/sec ±0.22% (74 runs sampled)
@hoangcung1804npm/voluptatibus-placeat-quo large array json-stringify x 248 ops/sec ±0.07% (90 runs sampled)
...
@hoangcung1804npm/voluptatibus-placeat-quo long string x 15,979 ops/sec ±0.09% (96 runs sampled)
...
@hoangcung1804npm/voluptatibus-placeat-quo short string x 30,585,790 ops/sec ±0.27% (97 runs sampled)
...
@hoangcung1804npm/voluptatibus-placeat-quo obj x 6,866,434 ops/sec ±0.11% (100 runs sampled)
...
@hoangcung1804npm/voluptatibus-placeat-quo date format x 1,836,188 ops/sec ±0.12% (99 runs sampled)
...
<a name="example"></a>
Try it out on RunKit: <a href="https://runkit.com/npm/@hoangcung1804npm/voluptatibus-placeat-quo">https://runkit.com/npm/@hoangcung1804npm/voluptatibus-placeat-quo</a>
## Example

```js
const fastJson = require('@hoangcung1804npm/voluptatibus-placeat-quo')
...
Optionally, you may provide to `@hoangcung1804npm/voluptatibus-placeat-quo` an option object as second parameter:
...
```
If the object to stringify is missing the required field(s), `@hoangcung1804npm/voluptatibus-placeat-quo` will throw an error.

<a name="missingFields"></a>
#### Missing fields
If a field *is present* in the schema (and is not required) but it *is not present* in the object to stringify, `@hoangcung1804npm/voluptatibus-placeat-quo` will not write it in the final string.
Example:
```javascript
const stringify = fastJson({
  title: 'Example Schema',
...
<a name="defaults"></a>
#### Defaults
`@hoangcung1804npm/voluptatibus-placeat-quo` supports `default` jsonschema key in order to serialize a value
...
#### Pattern properties
`@hoangcung1804npm/voluptatibus-placeat-quo` supports pattern properties as defined by JSON schema.
...
`@hoangcung1804npm/voluptatibus-placeat-quo` supports additional properties as defined by JSON schema.
...
`@hoangcung1804npm/voluptatibus-placeat-quo` supports the **anyOf** and **oneOf** keywords as defined by JSON schema. Both must be an array of valid JSON schemas. The different schemas will be tested in the specified order. The more schemas `stringify` has to try before finding a match, the slower it will be.
...
#### If/then/else
`@hoangcung1804npm/voluptatibus-placeat-quo` supports `if/then/else` jsonschema feature. See [ajv documentation](https://ajv.js.org/keywords.html#ifthenelse).
...
```
If you need to use an external definition, you can pass it as an option to `@hoangcung1804npm/voluptatibus-placeat-quo`.
Example:
```javascript
const schema = {
  title: 'Example Schema',
...
`@hoangcung1804npm/voluptatibus-placeat-quo` to handle arrays starts increasing exponentially, leading
...
@hoangcung1804npm/voluptatibus-placeat-quo large array default x 48.72 ops/sec ±4.92% (48 runs sampled)
@hoangcung1804npm/voluptatibus-placeat-quo large array json-stringify x 157 ops/sec ±0.76% (86 runs sampled)
...
To achieve low cost and high performance redaction `@hoangcung1804npm/voluptatibus-placeat-quo`
...
Users are responsible for sending trusted data. `@hoangcung1804npm/voluptatibus-placeat-quo` guarantees that you will get
...
itself. You need to have `@hoangcung1804npm/voluptatibus-placeat-quo` installed for the standalone code to work.
